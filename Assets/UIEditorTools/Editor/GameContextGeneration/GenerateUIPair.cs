...
{{
    ...
    {{
        ...
        {{
            ...
            public static void GenerateUIPair(string scriptFilename, string projectRootNamespace, string uiPairName, string uiControllerName, string uiViewName)
            {
            ...
            public static void CreateUIPairAsset(string uiPairName, string projectRootNamespace, string assetFilename, string uiViewGUID)
            {
                var pairType = AppDomain.CurrentDomain.GetAssemblies().SelectMany(a => a.GetTypes()).FirstOrDefault(t => t.FullName.EndsWith($"{projectRootNamespace}.Controllers.{uiPairName}"));
                if (pairType != null)
                {
